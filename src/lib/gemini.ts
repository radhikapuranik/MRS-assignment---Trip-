...
Every option's "perPerson" array must include exactly one entry for each of these ${preferences.length} people, in any order: ${preferences.map((p) => p.name).join(", ")}. The "verdict" field must be exactly one of "good fit", "partial fit", or "poor fit".`;
}
